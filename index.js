...
async function loginInstagram() {
    ...
    const ig_email_ele = await driver.wait(webdriver.until.elementLocated(webdriver.By.xpath(`//*[@id="loginForm"]/div/div[1]/div/label/input`)));//username
    ig_email_ele.sendKeys(ig_username);
    const ig_password_ele = await driver.wait(webdriver.until.elementLocated(webdriver.By.xpath(`//*[@id="loginForm"]/div/div[2]/div/label/input`)));//password
    ig_password_ele.sendKeys(ig_password);
    const ig_login_ele = await driver.wait(webdriver.until.elementLocated(webdriver.By.xpath(`//*[@id="loginForm"]/div/div[3]`)));//login
    ig_login_ele.click();

    await driver.wait(webdriver.until.elementLocated(webdriver.By.xpath(`//*[@id="react-root"]/section/nav/div[2]/div/div/div[3]/div`)));// 確認是否登入成功
